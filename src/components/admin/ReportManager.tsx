    ...
    <div className="space-y-8 pb-12">
      ...
      >
        <motion.div variants={item} className="bg-white p-6 rounded-[32px] shadow-sm border border-gray-100 flex items-center gap-5 relative overflow-hidden group">
           <div className="absolute right-0 bottom-0 opacity-5 group-hover:opacity-10 transition-opacity"><Calendar size={80} /></div>
           <div className="p-4 bg-primary/10 rounded-2xl text-primary relative z-10"><Calendar size={24} /></div>
           <div>
              <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Toplam Randevu</p>
              <h4 className="text-2xl font-black">{stats.total}</h4>
           </div>
        </motion.div>
        
        <motion.div variants={item} className="bg-white p-6 rounded-[32px] shadow-sm border border-gray-100 flex items-center gap-5 relative overflow-hidden group">
           <div className="absolute right-0 bottom-0 opacity-5 group-hover:opacity-10 transition-opacity"><TrendingUp size={80} /></div>
           <div className="p-4 bg-green-500/10 rounded-2xl text-green-500 relative z-10"><TrendingUp size={24} /></div>
           <div>
              <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Tahmini Ciro</p>
              <h4 className="text-2xl font-black">{stats.totalRevenue.toLocaleString('tr-TR')} ₺</h4>
           </div>
        </motion.div>

        <motion.div variants={item} className="bg-white p-6 rounded-[32px] shadow-sm border border-gray-100 flex items-center gap-5 relative overflow-hidden group">
           <div className="absolute right-0 bottom-0 opacity-5 group-hover:opacity-10 transition-opacity"><CheckCircle size={80} /></div>
           <div className="p-4 bg-blue-500/10 rounded-2xl text-blue-500 relative z-10"><CheckCircle size={24} /></div>
           <div>
        ...
        <motion.div variants={item} className="bg-white p-6 rounded-[32px] shadow-sm border border-gray-100 flex items-center gap-5 relative overflow-hidden group">
           <div className="absolute right-0 bottom-0 opacity-5 group-hover:opacity-10 transition-opacity"><Clock size={80} /></div>
           <div className="p-4 bg-orange-500/10 rounded-2xl text-orange-500 relative z-10"><Clock size={24} /></div>
           <div>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <motion.div 
          ...
          className="lg:col-span-1 bg-white p-6 md:p-8 rounded-[40px] shadow-sm border border-gray-100"
        >
              ...
              {stats.popularServices.map(([name, count], i) => (
                <div key={name} className="space-y-2">
                   <div className="flex justify-between items-center text-xs font-bold">
                      ...
                      <span className="text-primary">{count} Adet</span>
                   </div>
        ...
        >
           ...
           <div className="mt-8 p-4 bg-primary/5 rounded-3xl border border-primary/10">
              <p className="text-[10px] font-bold text-primary text-center italic leading-relaxed">
                "Onaylanmış randevuların oranı: %{Math.round((stats.confirmed / (stats.total || 1)) * 100)}"
              </p>
           </div>
        </motion.div>

        {/* Growth Trend - SCROLLABLE ON MOBILE */}
        <motion.div 
          initial={{ opacity: 0, x: 20 }}
          animate={{ opacity: 1, x: 0 }}
          className="lg:col-span-1 bg-white p-6 md:p-8 rounded-[40px] shadow-sm border border-gray-100"
        >
           ...
           <div className="overflow-x-auto pb-4">
              <div className="h-[250px] flex items-end justify-between gap-4 pt-10 min-w-[300px]">
                  {stats.monthlyData.map(([month, count], i) => {
                    const max = Math.max(...stats.monthlyData.map(d => d[1]), 1);
                    return (
                      <div key={month} className="flex flex-col items-center gap-3 flex-1 min-w-[40px]">
                         <div className="relative w-full flex flex-col items-center group h-full justify-end">
                            <div className="absolute -top-8 opacity-0 group-hover:opacity-100 transition-opacity bg-black text-white text-[10px] px-2 py-1 rounded font-bold mb-1 z-10 whitespace-nowrap">
                               {count} Randevu
                            </div>
                            <motion.div 
                              initial={{ height: 0 }}
                              ...
                              transition={{ duration: 1, delay: i * 0.1 }}
                              className="w-full max-w-[30px] bg-gradient-to-t from-primary to-primary-light rounded-t-lg hover:from-primary-dark hover:to-primary transition-colors cursor-pointer"
                            />
                         </div>
                         <span className="text-[9px] font-black text-gray-400 rotate-45 mt-4">{month}</span>
                      </div>
                    );
                  })}
              </div>
      ...
      <motion.div 
        ...
        animate={{ opacity: 1, y: 0 }}
        className="bg-white p-6 md:p-8 rounded-[40px] shadow-sm border border-gray-100"
      >
        <div className="flex justify-between items-center mb-6">
           <h3 className="text-lg font-black flex items-center gap-2">
              <Activity className="text-red-500" size={20} /> Son İşlemler
           </h3>
           <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest hidden md:block">Son 10 Kayıt</span>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
           {appointments.slice(0, 5).map((app, i) => (
             <div key={app.id} className="p-4 bg-gray-50 rounded-3xl border border-gray-100 flex flex-col justify-between h-28 hover:bg-white hover:shadow-lg transition-all">
                <div>
                   <h4 className="font-black text-xs truncate text-gray-800">{app.customer_name}</h4>
                   <p className="text-[9px] font-bold text-primary mt-1 truncate">{app.service_name}</p>
                </div>
           ...
           {appointments.length === 0 && (
             <div className="col-span-full text-center py-10 text-gray-400 text-sm font-bold">Henüz işlem kaydı yok.</div>
           )}
        </div>
      </motion.div>
    </div>
